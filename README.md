...
서버가 시작되면 브라우저에서 [http://127.0.0.1:56802](http://127.0.0.1:56802)를 엽니다. `56802`는 데모 서버의 기본 포트이므로 `--port`를 생략해도 같습니다.

루트의 `index.html`과 `main.html`은 고정된 `127.0.0.1`을 사용하지 않고, 사용자가 Machbase Neo에 접속한 현재 브라우저 URL의 호스트명과 데모 포트 `56802`를 조합하여 자동 이동합니다. 예를 들어 `http://192.168.0.20:5654`에서 진입 파일을 열면 `http://192.168.0.20:56802/`로 이동합니다. 자동 이동이 차단되면 화면에 계산된 주소의 링크가 표시됩니다.

접속 대상은 진입 파일의 쿼리 파라미터로 명시적으로 변경할 수도 있습니다.

```text
index.html?host=192.168.0.20&port=56802
```

다른 PC의 브라우저에서 접속하려면 서버가 외부 연결을 받을 수 있도록 `--host 0.0.0.0` 또는 서버의 실제 인터페이스 주소로 실행하고, 방화벽에서도 해당 포트를 허용해야 합니다. `127.0.0.1`로 실행한 서버는 서버와 같은 호스트에서만 접속할 수 있습니다.
...
```text
app/server.js          JSH HTTP 서버, 기본 포트 56802
