...
function manifest(args) {
  ...
  return withDb(args, function (conn) {
    ...
    const eventRows = queryAll(conn, `SELECT time, value FROM ${TABLES.timeline} WHERE name IN (?, ?) ORDER BY time`, metro.DATASET + '.' + metro.ASSET_ID + '.event.official.air_leak', metro.DATASET + '.' + metro.ASSET_ID + '.event.derived.early_warning');
